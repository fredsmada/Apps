...
function scene:create( event )
	...
	local background = display.newImageRect(nextGroup, "waterfall.jpg", 400, 700)
	...
	function setCalendar(currentMonth)
		local menuButton = display.newText(nextGroup, "Menu", display.contentCenterX, 10, native.systemFont, 25)
		menuButton:setFillColor(1,1,1)
		menuButton:addEventListener("tap", GoToMenu)

		local pointsButton = display.newText(nextGroup, "Trip Progress", display.contentCenterX, 50, native.systemFont, 25)
		...
		local monthDisplay = display.newText(nextGroup, monthSpot, display.contentCenterX, 115, native.systemFont, 35 )
		...
		for i=1,7 do
			if(i==1) then
				local boxes = display.newRect(nextGroup, 42, 198, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			else
				local boxes = display.newRect(nextGroup, 42*i, 198, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			end
		end
		...
		for j=1,7 do
			if(j==1) then
				local boxes = display.newRect(nextGroup, 42, 255, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			else
				local boxes = display.newRect(nextGroup, 42*j, 255, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			end
		end
		...
		for k=1,7 do
			if(k==1) then
				local boxes = display.newRect(nextGroup, 42, 310, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			else
				local boxes = display.newRect(nextGroup, 42*k, 310, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			end
		end
		...
		for l=1,7 do
			if(l==1) then
				local boxes = display.newRect(nextGroup, 42, 365, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			else
				local boxes = display.newRect(nextGroup, 42*l, 365, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			end
		end
		...
		for a=1,7 do
			if(a==1) then
				local boxes = display.newRect(nextGroup, 42, 475, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			else
				local boxes = display.newRect(nextGroup, 42*a, 475, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			end
		end
		...
		for m=1,7 do
			if(m==1) then
				local boxes = display.newRect(nextGroup, 42, 420, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			else
				local boxes = display.newRect(nextGroup, 42*m, 420, 40, 50)
				boxes:setFillColor(0.5,0.5,0.5,0.5)
				boxes:addEventListener("tap", displayDay)
			end
		end
		...
		for h=1, 7 do
			if (firstDay==h) then
				local dayNumber = h
				setDay = display.newText(nextGroup, "1", 42*firstDay, 195, native.systemFont, 27)
				startDays = '1'
			...
			if(dayDigit > 1) then
				local setDay = display.newText(nextGroup, h-firstDay+1, 42*h, 195, native.systemFont, 27)
				setDay:setFillColor(1,.8,0)
				startDays = startDays + 1
			end
			local dayDisplay = allDays[h]
			setDay = display.newText(nextGroup, allDays[h], 42*h, 160, native.systemFont, 18)
		...
		--Second row
		for i=8, 14 do
			local setDay = display.newText(nextGroup, i-firstDay+1, 42*(i-7), 258, native.systemFont, 27)
			setDay:setFillColor(1,.8,0)
		...
		--Third row
		for j=15, 21 do
			local setDay = display.newText(nextGroup, j-firstDay+1, 42*(j-14), 308, native.systemFont, 27)
			setDay:setFillColor(1,.8,0)
		...
		for k=22, 28 do
			if(28 >= k-firstDay+1) then
				local setDay = display.newText(nextGroup, k-firstDay+1, 42*(k-21), 363, native.systemFont, 27)
				setDay:setFillColor(1,.8,0)
		...
		for l=29, 35 do
			if(tot >= (l-firstDay+1)) then
				local setDay = display.newText(nextGroup, l-firstDay+1, 42*(l-28), 418, native.systemFont, 27)
				setDay:setFillColor(1,.8,0)
		...
		for q=36, 39 do
			if(tot >= (q-firstDay+1)) then
				local setDay = display.newText(nextGroup, q-firstDay+1, 42*(q-35), 473, native.systemFont, 27)
				setDay:setFillColor(1,.8,0)
	...
	local function nextMonth()
		...
		composer.gotoScene("menu", {time=800, effect="crossFade"})
		composer.removeScene("next")
		composer.gotoScene("next2", {time=800, effect="crossFade"})
	end

	--Show previous and next month arrows
	local leftArrow = display.newImageRect(nextGroup, "leftArrow.png", 25, 25)
	leftArrow.x = 20
	leftArrow.y = 115
	leftArrow:addEventListener("tap", previousMonth)

	local rightArrow = display.newImageRect(nextGroup, "rightArrow.png", 25, 25)
	rightArrow.x = 295
	rightArrow.y = 115
